...
const std::string LidarDriver::DEFAULT_IGNORE_ARRAY_STRING = "";
  ...
  , quit_driver_(false)
{
  ...
  nh_private_.param<std::string>("ignore_array", ignore_array_string_, DEFAULT_IGNORE_ARRAY_STRING);
  ...
  std::stringstream string_stream(ignore_array_string_);
  std::string token;
  while (std::getline(string_stream, token, ','))
    ignore_array_.push_back(std::atof(token.c_str()));
  ...
  if (ignore_array_.size() %2 != 0) {
    ROS_ERROR("Invalid ignore_array parameter (%s)", ignore_array_string_.c_str());
    exit(-1);
  }
  if (!std::all_of(ignore_array_.begin(), ignore_array_.end(), [](double value) {
    return (value >= -180 && value <= 180);
  })) {
    ROS_ERROR("Invalid ignore_array parameter (at least one value is out of permitted range)");
    exit(-1);
  }
      ...
      if (!reboot_required) {
        ...
        auto updateLaserScan = [&](const ldcp_sdk::ScanBlock& scan_block) {
          ...
          for (int i = 0; i < block_size; i++) {
            ...
            bool clipped = false;
            for (int j = 0; j < ignore_array_.size() / 2; j++) {
              double lower_bound = ignore_array_[2 * j] * M_PI / 180;
              double upper_bound = ignore_array_[2 * j + 1] * M_PI / 180;
              double current_angle = beam_index * M_PI * 2 / beam_count;
              if (current_angle >= lower_bound && current_angle <= upper_bound)
                clipped = true;
            }
            if (clipped)
              continue;
