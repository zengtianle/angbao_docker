...
public:
    ...
    void toPGM(std::string const& filename, double const& max_, double const& min_) const;
    ...
    inline const double getEntry(size_t x, size_t y) const {
        ...
        if(i > grid.size() or i < 0) return max_entry;
        // ROS_ASSERT(i < grid.size());
        return grid[i];
    }
    // inline const double getEntry(size_t num) const {
    //     return grid[num];
    // }
