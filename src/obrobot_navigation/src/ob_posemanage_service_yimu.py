    ...
    for i in xrange(len(dict_source['floor'])):
        ...
        for j in xrange(len(list_floor['destination'])):
            ...
            if list_destination['id'] == dict_pose['id']:
                ...
                dict_pose=list_destination
                dict_pose['floor_id']=list_floor['floor_id']
                flag_status = "SUCCESS: select pose"
                break#

        if flag_pos_exit == True:
            break
    if flag_pos_exit == False:
        flag_status = "FAIL:pose does not exist"
...
def handle_posemanage(req):
    ...
    # if (req.poseoperation.id == 100000 or req.poseoperation.id == 100001 or req.poseoperation.id == 100002 or req.poseoperation.id == 100003) and ((req.poseoperation.floor != 1) and (req.poseoperation.opt !="select")):
    #     return PoseManageResponse("Status: FAIL: It is not allowed to operate special points on non-1st floor'",posdefine)

    # if (req.poseoperation.id == 100000 or req.poseoperation.id == 100001 or req.poseoperation.id == 100002 or req.poseoperation.id == 100003) and (req.poseoperation.opt =="delete"):
    #     return PoseManageResponse("Status: FAIL: It is not allowed to delete special points'",posdefine)
    ...
    if (req.poseoperation.opt =="update") or (req.poseoperation.opt =="insert"):
        rospy.loginfo("waitForTransform")
        tflistener = tf.TransformListener()
        # tempnow = rospy.Time.now()
    ...
    with open("/home/" + getpass.getuser() + "/.ob/pos/OB_pose.json",'r') as f:
    ...
    if req.poseoperation.opt =="insert":
        ...
        if "SUCCESS" in flag_status:
            with open("/home/" + getpass.getuser() + "/.ob/pos/OB_pose.json","w") as f:
